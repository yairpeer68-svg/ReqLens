...
public final class BrowserHistoryActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) { super.onCreate(state); setContentView(buildUi()); refresh(); }
    ...
    private View buildUi() {
        ...
        adapter = new ArrayAdapter<BrowserRequest>(this,android.R.layout.simple_list_item_2,android.R.id.text1,items){
            @Override public View getView(int p, View c, ViewGroup g){View v=super.getView(p,c,g);BrowserRequest x=getItem(p);((TextView)v.findViewById(android.R.id.text1)).setText(x.toString());String host="";try{host=new URL(x.url).getHost();}catch(Exception ignored){}((TextView)v.findViewById(android.R.id.text2)).setText((x.mainFrame?"PAGE • ":"RESOURCE • ")+host+(x.mimeType.isEmpty()?"":" • "+x.mimeType));return v;}
        };
        list.setAdapter(adapter); list.setOnItemClickListener((a,v,p,id)->show(items.get(p))); root.addView(list,new LinearLayout.LayoutParams(-1,0,1));
