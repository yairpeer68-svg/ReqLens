...
public final class BrowserHistoryActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) { super.onCreate(state); ProjectRepository.init(this); BrowserHistoryRepository.init(this); RequestAnnotationRepository.init(this); setContentView(buildUi()); refresh(); }
    ...
    private View buildUi() {
        ...
        adapter = new ArrayAdapter<BrowserRequest>(this,android.R.layout.simple_list_item_2,android.R.id.text1,items){
            @Override public View getView(int p, View c, ViewGroup g){View v=super.getView(p,c,g);BrowserRequest x=getItem(p);RequestAnnotationRepository.Annotation an=RequestAnnotationRepository.get(x.method,x.url);((TextView)v.findViewById(android.R.id.text1)).setText((an.pinned?"★ ":"")+x.toString());String host="";try{host=new URL(x.url).getHost();}catch(Exception ignored){}String kind=RequestClassifier.kind(x.method,x.statusCode,x.mimeType,x.url);String meta=kind+" • "+(x.mainFrame?"PAGE • ":"RESOURCE • ")+host+(x.mimeType.isEmpty()?"":" • "+x.mimeType)+(an.tags.isEmpty()?"":" • "+an.tags);((TextView)v.findViewById(android.R.id.text2)).setText(meta);v.setBackgroundColor(RequestClassifier.color(kind));return v;}
        };
    ...
    private void exportHistory(){try{java.util.List<BrowserRequest>x=BrowserHistoryRepository.snapshotActiveProject();String json=HistoryExporter.browserJson(x,SuiteSettings.redact(this));java.io.File f=ExportManager.writeText(this,"reqlens-browser-"+System.currentTimeMillis()+".json",json);startActivity(Intent.createChooser(ExportManager.share(this,f,"application/json"),"Export browser history"));}catch(Exception e){Toast.makeText(this,"Export failed: "+e.getMessage(),Toast.LENGTH_SHORT).show();}}
